...
from dataset.dataset import AVADataset, HKPHIDataset
...
def main(config):
    ...
    if config.weights:
        ...
        print('Successfully loaded model weights from %s' % config.weights)
    ...
    if config.train:
        ...
        trainset = HKPHIDataset(csv_file=config.train_csv_file, root_dir=config.img_path, pkl_file=config.train_pkl_file, transform=train_transform)
        valset = HKPHIDataset(csv_file=config.val_csv_file, root_dir=config.img_path, pkl_file=config.val_pkl_file, transform=val_transform)
        ...
        for epoch in range(config.warm_start_epoch, config.epochs):
            ...
            for i, data in enumerate(train_loader):
                ...
                print('Epoch: %d/%d | Step: %d/%d | Training EMD loss: %.4f' % (
                    epoch + 1, config.epochs, i + 1, len(trainset) // config.train_batch_size + 1, batch_losses[-1]))
                writer.add_scalar('batch train loss', batch_losses[-1],
    ...
    if config.test:
        ...
        testset = HKPHIDataset(csv_file=config.test_csv_file, root_dir=config.img_path, pkl_file=config.test_pkl_file, transform=val_transform)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_csv_file', type=str, default='./data/HKPHI22/noisy_ratings_val.csv')
    parser.add_argument('--val_csv_file', type=str, default='./data/HKPHI22/noisy_ratings_test.csv')
    parser.add_argument('--test_csv_file', type=str, default='./data/HKPHI22/noisy_ratings_test.csv')
    parser.add_argument('--img_path', type=str, default='./data/HKPHI22/')
    parser.add_argument('--train_pkl_file', type=str, default='./data/HKPHI22/noisy_50_val_all.pkl')
    parser.add_argument('--val_pkl_file', type=str, default='./data/HKPHI22/noisy_50_test_all.pkl')
    parser.add_argument('--test_pkl_file', type=str, default='./data/HKPHI22/noisy_50_test_all.pkl')

    # training parameters
    parser.add_argument('--train', action='store_true')
    parser.add_argument('--test', action='store_true')
    parser.add_argument('--decay', action='store_true')
    parser.add_argument('--conv_base_lr', type=float, default=5e-4)
    parser.add_argument('--dense_lr', type=float, default=7e-3)
    ...
    # misc
    parser.add_argument('--ckpt_path', type=str, default='./ckpts_hkphi')
    ...
    parser.add_argument('--weights', type=str, default='./weights/ava_100_512.pth')
